...
- (void) testMissingSuperTemplate {
  ...
  [self writeFile:tmpdir param1:@"t2.stg" param2:templates2];
...
- (void) testNoPropertyNotError {
  ...
  [group setListener:errors];
  ST * st = [group getInstanceOf:@"t"];
  [st add:@"u" param1:[[[User alloc] init:32 param1:@"parrt"] autorelease]];
  [st render];
  NSString * expected = @"";
  NSString * result = [errors description];
  [self assertEquals:expected param1:result];
...
- (void) testHiddenPropertyNotError {
  ...
  [group setListener:errors];
  ST * st = [group getInstanceOf:@"t"];
  [st add:@"u" param1:[[[UserHiddenName alloc] init:@"parrt"] autorelease]];
  [st render];
  NSString * expected = @"";
  NSString * result = [errors description];
  [self assertEquals:expected param1:result];
...
- (void) testHiddenFieldNotError {
  ...
  [group setListener:errors];
  ST * st = [group getInstanceOf:@"t"];
  [st add:@"u" param1:[[[UserHiddenNameField alloc] init:@"parrt"] autorelease]];
  [st render];
  NSString * expected = @"";
  NSString * result = [errors description];
  [self assertEquals:expected param1:result];
...
- (void) testParallelAttributeIterationWithMissingArgs {
  ...
  [group setListener:errors];
  ST * e = [[[ST alloc] init:group param1:@"<names,phones,salaries:{n,p | <n>@<p>}; separator=\", \">"] autorelease];
  ...
  [self assertEquals:errorExpecting param1:[errors description]];
...
- (void) testStringTypeMismatch {
  ...
  STGroup * group = [[[STGroup alloc] init] autorelease];
  [group setListener:errors];
  ST * e = [[[ST alloc] init:group param1:@"<trim(s)>"] autorelease];
  [e add:@"s" param1:34];
  [e render];
...
- (void) testStringTypeMismatch2 {
  ...
  STGroup * group = [[[STGroup alloc] init] autorelease];
  [group setListener:errors];
  ST * e = [[[ST alloc] init:group param1:@"<strlen(s)>"] autorelease];
  [e add:@"s" param1:34];
  [e render];
